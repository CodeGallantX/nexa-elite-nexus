...
export const Announcements: React.FC = () => {
  ...
  const getCategoryIcon = (title: string) => {
    ...
    if (titleLower.includes('tournament') || titleLower.includes('championship')) return Trophy;
    if (titleLower.includes('meeting') || titleLower.includes('strategy')) return Users;
    if (titleLower.includes('schedule') || titleLower.includes('war')) return Calendar;
    if (titleLower.includes('maintenance') || titleLower.includes('server')) return AlertCircle;
    if (titleLower.includes('welcome') || titleLower.includes('member')) return Target;
    return Megaphone;
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-[#FF1F44] mb-1">{announcements.length}</div>
            <div className="text-sm text-gray-400">Total Posts</div>
          </CardContent>
        </Card>
        <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-400 mb-1">{recentCount}</div>
            <div className="text-sm text-gray-400">New (24h)</div>
          </CardContent>
        </Card>
        <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-yellow-400 mb-1">{weeklyCount}</div>
            <div className="text-sm text-gray-400">This Week</div>
          </CardContent>
        </Card>
        <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-green-400 mb-1">{announcements.filter(a => a.profiles?.role === 'admin').length}</div>
      ...
      {isLoading ? (
        ...
        <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground">Check back later for updates from the team!</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {announcements.map(announcement => {
            ...
            const daysSinceCreated = Math.floor((Date.now() - new Date(announcement.created_at).getTime()) / (1000 * 60 * 60 * 24));
              ...
              >
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <Badge className={getPriorityColor(announcement.created_at)}>
                ...
                <CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    <Badge variant="outline" className="border-white/30 text-gray-400">
                      {announcement.profiles?.role === 'admin' ? 'Official' : 'Community'}
                    </Badge>
