...
export const Loadouts: React.FC = () => {
  ...
  // Filter loadouts
  const myLoadouts = loadouts.filter(loadout => 
  ...
  const communityLoadouts = loadouts.filter(loadout => 
    loadout.player_id !== profile?.id &&
    loadout.is_public &&
    loadout.weapon_name.toLowerCase().includes(searchTerm.toLowerCase()) &&
    (modeFilter === 'all' || loadout.mode === modeFilter) &&
  ...
  const getModeColor = (mode: string) => {
    switch (mode) {
      case 'MP': return 'bg-blue-500/20 text-blue-400';
      case 'BR': return 'bg-orange-500/20 text-orange-400';
  ...
  const getWeaponTypeColor = (type: string) => {
    switch (type) {
      case 'Assault Rifle': return 'bg-red-500/20 text-red-400';
      case 'SMG': return 'bg-green-500/20 text-green-400';
      case 'Sniper': return 'bg-purple-500/20 text-purple-400';
      case 'LMG': return 'bg-yellow-500/20 text-yellow-400';
      case 'Shotgun': return 'bg-orange-500/20 text-orange-400';
      default: return 'bg-gray-500/20 text-gray-400';
    }
  };

  const weaponTypes = ['Assault Rifle', 'SMG', 'Sniper', 'LMG', 'Shotgun', 'Marksman Rifle', 'Pistol'];
  const modes = ['MP', 'BR'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground font-orbitron">Loadouts</h1>
          <p className="text-muted-foreground font-rajdhani">Create and manage your weapon loadouts</p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={isCreating} onOpenChange={setIsCreating}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="font-orbitron">Create New Loadout</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => setFormData(prev => ({ ...prev, weapon_type: value }))}
                    >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    {Object.keys(formData.attachments).map(attachment => (
                      <div key={attachment}>
                        <Label htmlFor={attachment} className="capitalize text-sm">
                          {attachment.replace(/([A-Z])/g, ' $1')}
                        </Label>
                        <Input
                          id={attachment}
                          value={formData.attachments[attachment as keyof typeof formData.attachments]}
                ...
                <div className="flex space-x-2">
                  <Button type="submit" disabled={saveLoadoutMutation.isPending}>
                    Create Loadout
                  </Button>
                  <Button
                    type="button"
      ...
      <div className="space-y-6">
        {!showCommunity ? (
          <>
            <div>
              <h2 className="text-xl font-semibold text-foreground font-orbitron mb-4">My Loadouts ({myLoadouts.length})</h2>
              {myLoadouts.length === 0 ? (
                <Card className="bg-card/50 border-border/30">
                  <CardContent className="p-8 text-center">
                    <Target className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
                    <p className="text-muted-foreground font-rajdhani">No loadouts created yet.</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {myLoadouts.map((loadout) => (
                    <Card key={loadout.id} className="bg-card/50 border-border/30 hover:border-primary/30 transition-colors">
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          <div>
                            <CardTitle className="text-lg font-orbitron">{loadout.weapon_name}</CardTitle>
                      ...
                      <CardContent className="pt-0">
                        {loadout.description && (
                          <p className="text-sm text-muted-foreground mb-3 font-rajdhani">{loadout.description}</p>
                        )}
                        <div className="flex gap-2">
                          <Button
                            size="sm"
          ...
          <>
            <div>
              <h2 className="text-xl font-semibold text-foreground font-orbitron mb-4">Community Loadouts ({communityLoadouts.length})</h2>
              {communityLoadouts.length === 0 ? (
                <Card className="bg-card/50 border-border/30">
                  <CardContent className="p-8 text-center">
                    <User className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
                    <p className="text-muted-foreground font-rajdhani">No community loadouts found.</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <Card key={loadout.id} className="bg-card/50 border-border/30 hover:border-primary/30 transition-colors">
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          <div>
                            <CardTitle className="text-lg font-orbitron">{loadout.weapon_name}</CardTitle>
                            <p className="text-sm text-muted-foreground font-rajdhani">by {loadout.profiles.ign}</p>
                            <div className="flex gap-2 mt-2">
                              <Badge className={getModeColor(loadout.mode)}>{loadout.mode}</Badge>
                              <Badge className={getWeaponTypeColor(loadout.weapon_type)}>{loadout.weapon_type}</Badge>
                            </div>
          ...
          {selectedLoadout && (
            <div className="space-y-4">
              ...
              {selectedLoadout.description && (
                <p className="text-muted-foreground font-rajdhani">{selectedLoadout.description}</p>
              )}

              <div>
                <h3 className="text-lg font-semibold font-orbitron mb-3">Attachments</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {Object.entries(selectedLoadout.attachments || {}).map(([key, value]) => (
                    value && (
                      <div key={key} className="flex justify-between items-center p-2 bg-background/30 rounded">
